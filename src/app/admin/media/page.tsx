...
export default function MediaLibrary() {
  ...
  return (
    <div className="max-w-7xl mx-auto">
      ...
      <div className="mb-6">
        <div className="flex flex-col gap-4">
          ...
          <button
            onClick={fetchFiles}
            disabled={loading}
            className="flex items-center justify-center gap-2 px-4 py-3 rounded-lg border border-surface-300 dark:border-surface-600 hover:bg-surface-100 dark:hover:bg-surface-700 transition-colors w-full sm:w-auto active:scale-[0.98]"
          >
      ...
      {Object.keys(categoryStats).length > 0 && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
          {Object.entries(categoryStats).map(([cat, count]) => {
            ...
            return (
              <div key={cat} className="card preset-filled-surface-100-900 p-3">
                <div className="flex items-center gap-2">
                  <Icon className="w-4 h-4 text-surface-400" />
                  <p className="text-xs text-surface-500 capitalize">{cat}s</p>
                </div>
        ...
        <div className="flex items-center gap-2 overflow-x-auto pb-2 sm:pb-0 sm:flex-wrap scrollbar-hide">
          {['all', 'image', 'video', 'document', 'audio'].map((cat) => (
            <button
              ...
              className={`flex-shrink-0 px-4 py-2.5 rounded-full text-sm font-medium transition-colors active:scale-[0.98] ${
                filter === cat
      ...
      {/* Error */}
      {error && (
        <div className="mb-6 p-4 rounded-lg bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 text-red-600 dark:text-red-400 flex items-start gap-3 text-sm">
      ...
      {!loading && filteredFiles.length === 0 && (
        <div className="card preset-filled-surface-100-900 text-center py-12 px-6">
          <div className="w-14 h-14 mx-auto mb-4 rounded-full bg-surface-200 dark:bg-surface-700 flex items-center justify-center">
            <Image className="w-7 h-7 text-surface-400" />
          </div>
            ...
            return (
              ...
              >
                <div className="flex gap-3 p-3">
                  {/* Thumbnail */}
                  <div className="w-20 h-20 flex-shrink-0 rounded-lg bg-surface-200 dark:bg-surface-700 overflow-hidden">
                    {file.category === 'image' ? (
                      <img
                        src={file.url}
                        alt={fileName}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-3">
                      <button
                        ...
                        className="flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg bg-surface-200 dark:bg-surface-700 text-surface-700 dark:text-surface-300 text-xs font-medium transition-colors active:scale-[0.98]"
                      >
                        {copiedUrl === file.url ? (
                          <>
                      ...
                      <button
                        ...
                        className="flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400 text-xs font-medium transition-colors active:scale-[0.98]"
                      >
